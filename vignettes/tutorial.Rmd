...
```{r plot_rabimo_results,  fig.width = FIGURE_WIDTH}
...
- $e_{nat}$ and $e_{urb}$ = evaporation for the natural and urban state in mm, respectively,
- $i_{nat}$ and $i_{urb}$ = infiltration for the natural and urban state in mm, respectively,
- $r_{nat}$ and $r_{urb}$ = runoff for the natural and urban state in mm, respectively,
...
Calculate the deviation from the natural state in %:
...
The function `claculate_delta_w` returns a dataframe with two columns: 
- the code of the areas
- the value of $\Delta W$

If the water balance results were provided together with their geographical 
information, $\Delta W$ can be visualised:

```{r}
# plot deviation from natural water balance
delta_w_zoom <- kwb.rabimo::crop_box(
  delta_w, 
  xoffset = 0.35, 
  yoffset = 0.5, 
  xscale = 0.07, 
...
plot(delta_w_zoom[, "delta_w"], main = "Deviation from natural water balance in %")
```
